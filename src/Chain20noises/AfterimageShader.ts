   ...
   fragmentShader: glsl`
	...
	//   }
		...
		if (alive1 && (n ==2 || n == 3)){
			...
			next1 = 1;
			inc += sin(cnoise2(uv*10. + sin(time*.01)*4. + time*.01))*.1;
			// inc -= 0.0002;
		...
		if (inc <= inc + thresh || inc >= inc - thresh){
			...
			//inc -= 0.00001;
			next1 = 0;
			 inc += mix(0.0001,0.002,((snoise3(vec3(sin(uv.x + uv.y + time),uv.y*20.,time)))+ sin(time*.5)+1.5)*.2 );

			inc /= mix(1.,length(uv - 0.5)+.7,0.001 + sin(time*.25)*.007)*.9999;
