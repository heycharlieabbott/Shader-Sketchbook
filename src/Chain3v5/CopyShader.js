...
const CopyShader = {
	...
	uniform float opacity;
		...
		void main() {
			...
			uv += mix(cnoise2(uv*8.)*.002,cnoise2(uv*80.)*.05,0.01);

			float cir = length(uv - 0.5);
			cir = mix(cir*1.2 + cnoise2(uv*4000.)*.0002 / cir,sdBox(uv-0.5, vec2(.04)),4.2);
			float cir2 = cir;

			vec2 gv = fract((uv*990.)) * (1.0-cir*7.);
			gv -= 0.5;
			gv = smoothstep(-.12,.15,abs(gv));
			float g = (gv.x + gv.y)*.5;

			float mask = smoothstep(0.1,.7,cir);
			float mask2 = smoothstep(0.5,.8,cir);


			cir2 = smoothstep(-.11,.4,cir2);
