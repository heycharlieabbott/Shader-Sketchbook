...
//
    ...
    float cir = smoothstep(0.,1.,pow(length(uv - 0.5),.8));
    gl_FragColor = max(texelNew,texelOld*.8 + sin(time)*.2 * cir * snoise3(texelOld.xyz*100. + random(uv)));
    // gl_FragColor = vec4(cir);
