 ...
 */
 ...
 export const AfterimageShader: IAfterimageShader = {
   ...
   fragmentShader: glsl`
    ...
    float when_gt1( float x, float y ) {
			return max( sign( x - y ), 0.0 );
		}
    ...
    texelOld.w += when_gt1(texelOld.w*1.,.1); 

    // texelNew.xy *= rotate2d((texelOld.w));

    vec2 noisecoords = vUv * 1.;

    noisecoords.x += texelOld.x*3000. + time*.2;

    float noiser = cnoise2(noisecoords);

    noiser = smoothstep(.1,1.,noiser)*.005;

    
 
    gl_FragColor.a = 1.;

    gl_FragColor = max(texelNew,texelOld*.99);
