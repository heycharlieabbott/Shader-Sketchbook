...
#define SAMPLES 20
  ...
  void main() {
    ...
    cir = pow(cir,(sin(iTime*.01)+.1)*.5)*10.9;
    cir = fract(log(cir));
    ...
    cir = (cos(cir* 2.1 + sin(iTime*.1)*(1.5 + sin(iTime*.5))));

    //cir = mix(cir,.002/cir,float(i)*1. * sin(iTime*.01)*.05);
    // cir += sin(uv.y*.1 + iTime)*.9;

    // circol.xy = mix(circol.xy, circol.xy * rotate2d(cir + smoothstep(0.8,.1,cnoise2(uv*10.))),(sin(iTime)*.5));
    // circol.y = 1.-circol.x;
    //circol *= 1.-clamp(0.,1.,dot(circol.xz + cir,circol.yz));

    circol = pal( cir * length(uv - 0.5) + iTime*.1, vec3(.7,0.1,0.1),vec3(0.5,0.5,0.5),vec3(1.,1.,1.0),vec3(0.0,0.33,0.67) );
    

    cir-= smoothstep(.2 + (sin(iTime*.7)+1.)*.5,-.1,length(uv - 0.5))*1.;
  }

    vec3 col = (vec3(cir) + circol)/2.;
    
    vec3 avg = vec3(col.x + col.y + col.z)/3.;
     col = mix(col,avg,vec3(length(vUv-0.5)*2.));
    col -= smoothstep(0.,1.,length(vUv - 0.5))*.1;
    //col -= cos(smoothstep(.2,-.1,(length(vUv - 0.5)))*10. + iTime)*1.;
