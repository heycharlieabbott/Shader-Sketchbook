...
//    vec4 monocol = texture2D( tNew, vUv );
    ...
    // texelNew.xy *= rotate2d(time);
    // texelOld.yz *= rotate2d(time);
    // texelNew.xy *= smoothstep(0.8,.7,texelNew.xy);

    texelOld.w += when_gt1(texelOld.w*1.,.1);    
 
  //  texelNew = mix(texelNew,monocol,sin(time)*2.);
